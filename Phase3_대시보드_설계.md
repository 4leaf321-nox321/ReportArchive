...
## 방향 전환 (2026-06-14) — 콘텐츠 수치 → 메타데이터 통계
- 콘텐츠 수치 지표(템플릿 본문 숫자)는 **템플릿마다 차원이 제각각 → 조합 폭발**,
  대시보드 구성이 난감 + 템플릿 버전 취약. 그래서 **보고서 메타데이터 통계로 전환**.
- **3B-2(콘텐츠 값 추출·집계) 보류.** `content_metrics` 는 당분간 `[]` 유지.
- **TemplateMetric(3B-1) 휴면**: 모델·마이그레이션·관리 API·페이지는 남기되 사이드바
  메뉴만 숨김(추후 "심층 분석" 용도로 재검토 가능).
- **신규: 메타데이터 분포(v0.47.0)** — `/api/dashboard` 응답에 `distributions`(차원별
  보고서 수) 추가. 차원 = 엔티티 축(모델·부품·개발단계·불량·신뢰성시험·…) + 종류
  + 템플릿. 프런트는 "분포" 카드 1개 + 차원 선택 드롭다운으로 표시(기존 엔티티
  커버리지 카드 대체). 모두 통제 어휘/메타라 집계 깔끔·전 보고서 공통.
