...
const COMPOSITE_VIEWER_SCRIPT = [
  ...
  '  });',
  '',
  '  // scale-to-fit — when the captured root is wider than the visible',
  '  // stage, shrink it with CSS zoom so it fits and stays centered (zoom',
  '  // reflows layout, so the root`s `margin:0 auto` keeps centering it).',
  '  // Roots narrower than the stage stay at 1:1 and center normally.',
  '  var root = shell.querySelector(".composite-detail-root");',
  '  function fitRoot() {',
  '    var stage = shell.querySelector(".cv-stage");',
  '    if (!stage || !root) return;',
  '    var avail = stage.clientWidth - 16;',
  '    if (avail <= 0) return;',
  '    root.style.zoom = "";',
  '    var natural = root.offsetWidth;',
  '    if (natural > 0 && natural > avail) root.style.zoom = String(avail / natural);',
  '  }',
