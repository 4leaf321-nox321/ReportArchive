...
- **Phase 4 잔여** — 자동 요약(`summarize_report`)·다중홉 에이전트·대화형은 완료.
  **아직 없음: 보고서 비교 · 작성 보조 · FMEA 연관 문서 추천**(벡터 유사도 — 도메인 킬러 기능).
- ~~(선택) 프론트 검색 UI 시맨틱 토글~~ → **완료**(SearchPage 모드 선택).
- ⚠️ **운영 미가동 — 지능화 노브 8개가 전부 off.** 코드·배선은 다 있는데 아무도 켠 적이
  없다(`app_settings` override 0건). 관리자 → AI설정 → **검색 튜닝** 탭에서 켤 수 있다.
  공짜(LLM 미사용)인 **별칭 확장·최신성/권위 가중**부터 켜볼 것. 나머지 5개(재랭킹·HyDE·
  질문분해·집계라우팅·근거검증)는 LLM 호출이 늘어 지연·비용이 붙는다 —
  `[일부] AI검색_지능화_로드맵_설계.md` §9 의 "토글 기본값 결정" 이 아직 미결.
