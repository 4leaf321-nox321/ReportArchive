...
번호로 가리키면 순서가 바뀌어도 **렌더할 때 다시 매겨진다**(번호는 저장하지 않는다).

rich_text `items` 의 한 줄에 `text` 대신 `html` 을 주고 앵커를 넣는다:

```json
{"depth": 0, "text": "결과는 표 1 과 같다.",
 "html": "<p>결과는 <a data-mention-type=\"block\" data-mention-id=\"tbl\" data-mention-page=\"0\" href=\"#\">표 1</a> 과 같다.</p>"}
```

- ⚠️ **`data-mention-page` 는 0부터** 센다(1쪽 = `"0"`). 도구 인자의 `page` 는 1부터라
  **가장 자주 틀리는 곳**이다 — 틀리면 화면에 "(삭제된 항목)" 으로 뜬다.
- 앵커 안 글자("표 1")는 렌더 시 **현재 번호로 자동 교체**된다. `text`(평문 폴백)에는
  사람이 읽을 문장을 그대로 쓴다.
- 가리킬 수 있는 것: 그림·표·비교표·키-값·RACI·FMEA·수식·목록·첨부·영상·임베드·문서.
  제목·본문은 대상이 아니다.
- 다 쓰면 **`get_report_outline` 이 깨진 참조를 잡아준다** — 반드시 확인하라.
...
<!--@ check -->
## 자기 점검 — 네가 못 보는 것
너는 **완성된 화면을 볼 수 없다.** 구조상 멀쩡해도 화면에선 깨져 보일 수 있다.
작성·수정을 마치면 `get_report_outline(report_id)` 로 점검하고, `issues` 를
**사용자에게 그대로 전하라**(네가 임의로 채우지 말고).

`issues` 가 잡는 것:
- 보이는데 **비어 있는 블록** / 블록이 하나도 없는 **빈 페이지**
- **파일이 없어 깨지는 곳** — 본문은 이미지·첨부를 참조하는데 파일이 없다
- **깨진 상호참조** — 가리키는 블록이 없다(화면엔 "(삭제된 항목)")
- **값이 하나뿐인 표·차트** — 만들다 만 것일 때가 많다(이미지·수식 한 개는 정상)

비어 있는 게 의도된 자리도 있다(사람이 채울 칸). 그러니 "○쪽 △△가 비어 있습니다"
라고 **알리고 물어라** — 추측으로 채우면 없는 사실을 지어내게 된다.
