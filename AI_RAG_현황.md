...
- **Phase 3** — B300 LLM 연결 → RAG Q&A(관련 청크 검색 → LLM이 출처 인용 답변).
  B300 ↔ 앱서버 네트워크 연결이 선결.
- **Phase 4** — 에이전트: 자동 요약(`summarize_report` 핸들러)·보고서 비교·작성 보조·
  **FMEA 연관 문서 추천**(벡터 유사도 — 도메인 킬러 기능).
- (선택) 프론트 검색 UI 시맨틱 토글.
