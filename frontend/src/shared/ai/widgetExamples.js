...
export const WIDGET_PROMPT_EXAMPLES = [
  ...
  },
  {
    types: ['comparison'],
    body: [
      '### comparison (비교 표 — AS-IS / TO-BE / 다중 CASE)',
      ...
      '- `rows[]` = 비교 항목. 각 행의 `kind` 가 `"text"` (자유 텍스트) | `"image"` 중 하나.',
      '- `values` 객체의 키는 cases 의 `key` 와 정확히 일치. 값은 `kind="text"` 일 때 문자열(빈 칸은 키 생략).',
  ...
  },
  {
    types: ['quadrant'],
    body: [
      '### quadrant (2x2 매트릭스 — SWOT / Eisenhower / BCG / Risk)',
