...
### Phase 5 — 편집(수정) 기능 갭 분석 (TODO, 우선순위순)
> `update_report_draft` 로 **기존 문서를 수정**할 때 현재 부족한 점. 실제 사용 점검(2026-06-15)으로 도출.
> 도구 인자 기준: 현재 수정은 `title`·`blocks`(블록 통째 교체)·`extra_blocks`·`remove_blocks`·`block_sections`·`page`(append 포함)·`pages`(전체 교체)만 받는다.

**⚠️ 큰 갭**
- [ ] **위젯 내 "부분 수정"(블록 통째 덮어쓰기만)** — `blocks` 로 주면 그 블록 content **전체가 교체**됨. "표에 한 줄 추가"·"특정 셀만 수정"이 불가 → AI 가 `get_report` 로 전체를 읽어 **표 전체를 재전송**해야 함(토큰 낭비·실수 위험). 행 추가/셀 패치 같은 **세분 연산 없음**. ⇒ *체감 1순위.* 예: `append_rows`/`patch_rows`.
- [ ] **보고서 레벨 속성 수정 불가** — MCP 수정은 `title`+`pages(content)` 만 바꿈. **태그·보고일(report_date)·보고서 종류·엔티티 태그·표시설정(폭/슬라이드 등)** 은 못 건드림. 백엔드 `update_report` 는 이미 지원 → **MCP 인자 노출만** 하면 됨. ⇒ *저비용 고효율(권장 착수점).*

**🟡 중간**
- [ ] **페이지/블록 재배치·삭제 제한** — 페이지는 **추가(append=page+1)·전체교체**만; 중간 삽입·삭제·순서변경 불가. 한 페이지 내 **블록 순서**도 AI 지정 불가(템플릿 순서+추가분 고정).
- [ ] **레이아웃 제어 불가** — 배치는 자동만(수동 레이아웃 *보존*은 하나 *지정*은 불가). "좌우로"·"전폭" 같은 지시 못 받음.
- [ ] **스타일 지정 불가** — 글자 색/스타일(`props_overrides` text_style 등) MCP 로 못 바꿈(내용만).
- [ ] **MCP 로 되돌리기(undo) 불가** — 수정 시 버전 이력은 남지만 **버전 목록/복원 도구 없음**. AI 오작동 시 사람이 UI 에서 복구. ⇒ `list_versions`/`restore` 도구 검토.

**ℹ️ 알려진 제약(설계상/별도 과제)**
- [ ] **파일 기반 위젯**(image·attachment·cad_3d·video) — 업로드 경로 없음(비워 둠).
- [ ] **동시성** — 편집 락 보유 시 거부만(force/대기 없음 — 안전 위주, 의도된 동작).
- [ ] **미리보기(dry-run) 없음** — 검증은 하지만 적용 전 변경 미리보기는 없음(바로 초안 반영).
...
### Phase 7+ — 보강 로드맵 → 별도 문서로 분화
> Phase 5 갭 분석 이후 남은 것과 Phase 6 으로 새로 생긴 빚(게시글 수정을 열었는데 되돌릴
> 수단이 없음)을 묶어 **`docs/[미구현] MCP_보강_로드맵_설계.md`** 로 분화했다.
> 네 갈래: **A 안전망**(버전 목록·복원·감사 표식·dry-run) · **B 협업 통로**(댓글·알림) ·
> **C 세밀 편집**(표 행 추가/셀 패치 + expected_revision) · **D 자기 점검·산출물**
> (outline·종합보고). 아래 Phase 5 목록 중 ①보고서 레벨 속성은 **이미 구현됨**(체크 누락).
