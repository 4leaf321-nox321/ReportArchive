...
// ─────────────────────────────────────────────────────────────────────────── //
// AddLinkForm — 보고서 검색 + 관계 유형 + 메모 + 추가                        //
// ─────────────────────────────────────────────────────────────────────────── //
...
/**
 * 검색 가능한 single-select combobox — 옵션 수가 늘어나도 typing 으로
 * 좁힐 수 있다. 작성자/게시조직 필터에서 작은 trigger 와 popover 안에
 * cmdk Command 를 띄움. mruKey 가 주어지면 최근 선택값을 localStorage 에
 * 저장하고 드롭다운 상단에 「최근」 그룹으로 분리해 보여줌.
 *
 *  @param {object}   props
 *  @param {React.ComponentType=} props.icon          trigger 좌측 아이콘
 *  @param {string}   props.label                     trigger 라벨 ("작성자")
 *  @param {string=}  props.value                     선택값 ('' = 전체)
 *  @param {{value:string,label:string,count?:number}[]} props.options
 *  @param {string}   props.allLabel                  "전체" 표기
 *  @param {string}   props.searchPlaceholder         CommandInput placeholder
 *  @param {(next:string)=>void} props.onChange
 *  @param {string=}  props.mruKey                    localStorage key (선택)
 */
function FilterCombo({
  ...
  // 최근 선택값 중 현재 옵션 풀에 여전히 살아있는 것만. (작성자 탈퇴 /
  // 게시 조직 삭제 같은 경우엔 자연스럽게 빠짐.)
...
function AddLinkForm({ reportId, kinds, onAdd }) {
  ...
  // slug → kind 매핑. link 대상 적격성 (조직 보고서 || 게시된 개인 보고서)
  // 판정에 쓰임. 백엔드도 같은 정책을 강제하므로 클라 필터는 UX 안내용.
  const [workspaceKindBySlug, setWorkspaceKindBySlug] = useState({})
  // 작성자/게시 옵션 facets — 백엔드 /linkable-facets 한 번 호출로 받음.
  // 응답이 보고서 본문에 비해 가벼워 (옵션만) picker open 비용을 줄임.
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // (예: facets 가 500 이어도 보고서 검색은 가능).
      const [reportsRes, workspacesRes, facetsRes] = await Promise.allSettled([
        // 시스템 전체 linkable 풀 — actor 워크스페이스 밖 보고서도 포함.
        listLinkableReports(),
        listWorkspaces(),
        listLinkableFacets(),
      ])
  ...
  // 적격 후보 (link 대상 적격성 검증 통과한 것만 — 조직 보고서 ||
  // 어딘가에 게시된 보고서). 필터/검색은 그 다음 단계에서.
  ...
  }, [allReports, reportId, workspaceKindBySlug])

  // 작성자 / 게시조직 옵션은 백엔드 facet 응답 그대로. count 까지 같이
  // 노출해 사용자가 모집단 크기를 직관적으로 인식할 수 있게 한다.
  ...
  // 최종 filtered — 텍스트(제목·작성자·게시조직명)·기간·작성자·게시조직 다 적용.
